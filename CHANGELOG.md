...
All notable decisions and structural changes to TapTim.e are recorded here. Full architecture rationale lives in `ADO/01_Architecture/` and `ADO/99_Archive/00_Core/Decision_Log.md`; this file is a chronological summary, not a replacement for either.
...
- EP-006 Agent Operations Framework and official startup sequence (`ADO/99_Archive/README.md`).
...
- Superseded the pre-EP-006/EP-007 `AI Technical Lead Charter` and an earlier strategic review with `ADO/99_Archive/README.md`, EOM-001 and AGR-001. Note: on this branch's history, `ADO/99_Archive/` currently contains no files (only `.gitkeep`) — the superseded documents were archived on a separate branch (`architecture/ep-002-feature-blueprint-standard` and variants) and have not been carried into `main`. See Repository Maintenance Sprint 002 for this finding.

### Resolved
- ADR-0007 (Technology Platform Baseline): Approved (previously tracked here as "Not Decided").

2026-08-22 — Prozess-Reset, siehe ADO/DECISIONS.md.
