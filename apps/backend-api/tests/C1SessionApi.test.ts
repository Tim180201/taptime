...
const configuredOfflineLeaseConnectionString = connectionStringFor(
  installerConnectionString,
  'taptime_offline_lease_runtime',
  'offline-lease-local-synthetic-only',
);
const configuredOfflineEventConnectionString = connectionStringFor(
  installerConnectionString,
  'taptime_offline_event_runtime',
  'offline-event-local-synthetic-only',
);
const configuredOfflineReconciliationConnectionString = connectionStringFor(
  installerConnectionString,
...
const configuredTimeReviewReadConnectionString = connectionStringFor(
  installerConnectionString,
  'taptime_time_review_read_runtime',
  'time-review-read-local-synthetic-only',
);
const configuredTimeReviewWriteConnectionString = connectionStringFor(
  installerConnectionString,
...
beforeAll(async () => {
  ...
  runtime = createBackendApiRuntime({
    ...
    timeReviewReadDatabaseUrl: configuredTimeReviewReadConnectionString,
    timeReviewWriteDatabaseUrl: configuredTimeReviewWriteConnectionString,
...
describe('versioned C1 foundation', () => {
  it('uses exactly migrations 001 through 018 and reruns the ledger cleanly', async () => {
    const migrations = await loadMigrations();
    expect(migrations.map(({ version }) => version)).toEqual(['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018']);
    ...
    expect(ledger.rows.map(({ version }) => version)).toEqual(['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018']);
    expect(ledger.rows.every(({ checksum }) => /^[0-9a-f]{64}$/.test(checksum))).toBe(true);
    await expect(migrate(installerPool)).resolves.toEqual({
      applied: [], alreadyApplied: ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018'],
    });
  });
});
