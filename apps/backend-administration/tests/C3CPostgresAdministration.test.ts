...
describe('migration 007, roles and database contracts', () => {
  it('records exactly immutable migrations 001 through 020 and reruns without changes', async () => {
    ...
    async ({ kind, role }) => {
      ...
      try {
        ...
        await expect(applyMigrationSet(dirtyPool, migrations.slice(6))).resolves.toEqual({
          applied: ['007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018', '019', '020'],
          alreadyApplied: [],
        });
      } finally {
