...
export async function resetMigrateAndPrepare(
  ...
  if (result.applied.join(',') !== DA2_EXPECTED_MIGRATIONS.join(',')) {
    throw new Error('DA2 requires a clean migration set 001 through 021');
  }
...
export async function insertBulkStoppedEntries(
  ...
  const stopEventIds: string[] = [];
  const startedAts: string[] = [];
  const stoppedAts: string[] = [];
  const startEpochMilliseconds = Date.parse('2026-07-22T00:00:00Z');
  const stopEpochMilliseconds = Date.parse('2026-07-22T00:30:00Z');
  for (let value = 1; value <= count; value += 1) {
    const suffix = String(value).padStart(12, '0');
    entryIds.push(`61000000-0000-4000-8000-${suffix}`);
    startEventIds.push(`51000001-0000-4000-8000-${suffix}`);
    stopEventIds.push(`51000002-0000-4000-8000-${suffix}`);
    startedAts.push(new Date(startEpochMilliseconds + value * 1_000).toISOString());
  ...
  try {
    ...
    if (
      ...
      || counts.time_entry_count !== count
      || counts.decision_count !== count * 2
      || counts.complete_history_count !== count
    ) {
