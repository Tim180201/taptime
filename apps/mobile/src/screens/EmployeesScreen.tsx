...
export function EmployeesScreen({employees,scope,locationsEnabled}: {
  ...
  return <Screen title="Mitarbeiter"><ScrollView contentContainerStyle={{gap:16,paddingBottom:24}}>
    <Card><Text style={{fontSize:36,fontWeight:'800'}}>{state.summary.runningCount} / {state.summary.totalCount}</Text>
      <Text>gerade aktiv · {scope.kind==='organization' ? 'Betrieb' : scope.locationName}</Text>
      <Text style={{fontSize:12,color:mobileTokens.color.textMuted}}>Stand {formatOwnTimeTimestamp(state.summary.serverTime)}</Text></Card>
    <View style={{flexDirection:'row',gap:12}}>{[true,false].map(active=><TouchTarget key={String(active)} accessibilityRole="tab" accessibilityLabel={active ? 'Aktiv' : 'Inaktiv'}
      ...
      style={{flex:1,minHeight:48,padding:12,borderRadius:12,backgroundColor:state.filter===active ? mobileTokens.color.accent : mobileTokens.color.surface}}>
    ...
    {state.summary.people.map(person=><TouchTarget key={person.membershipId} accessibilityRole="button" accessibilityLabel={`${person.displayName}, ${person.isRunning ? 'aktiv' : 'inaktiv'}`} onPress={()=>employees.openPerson(person)}
      style={{minHeight:72,padding:16,gap:12,flexDirection:'row',alignItems:'center',backgroundColor:mobileTokens.color.surface,borderRadius:16}}>
      ...
      <View style={{flex:1}}><Text style={{fontWeight:'800'}}>{person.displayName}</Text>
        <Text style={{fontSize:12,color:mobileTokens.color.textMuted}}>{person.isRunning ? `seit ${formatClock(Date.parse(person.runningSince!))} · ${person.runningTargetDisplayName}` : 'Gerade inaktiv'}</Text></View>
