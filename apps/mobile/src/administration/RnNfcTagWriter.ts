...
/** Android setup only. The preceding scan has started NFC and retained the native tag. */
export class RnNfcTagWriter implements NfcTagWriter {
  private flight: Promise<TagWriteResult> | null = null;
  private generation = 0;

  constructor(private readonly packageName: string | null | undefined) {}
  ...
  write(canonicalPayload: string, uri: string): Promise<TagWriteResult> {
    ...
    const flight = this.performWrite(canonicalPayload, uri, this.packageName, this.generation).finally(() => {
