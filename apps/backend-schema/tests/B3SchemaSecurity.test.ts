...
describe('B3 deterministic migration system', () => {
  ...
  it('contains the approved tables and effective-record views', async () => {
    ...
    const declaredRelations = new Set((await loadMigrations()).flatMap(migration =>
      [...migration.sql.matchAll(/CREATE\s+(?:OR\s+REPLACE\s+)?(?:TABLE|VIEW)\s+(?:IF\s+NOT\s+EXISTS\s+)?taptime_server\.([a-z0-9_]+)/gi)]
        .map(match => match[1]!),
    ));
    expect(result.rows.map((row) => row.table_name)).toEqual([...declaredRelations].sort());
  });
