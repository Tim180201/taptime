...
describe('InMemoryCustomerRepository (DT-020)', () => {
  ...
  it('does not find a Customer saved under a different id', async () => {
    const repository = new InMemoryCustomerRepository();
    await repository.save({ id: CustomerId('customer-1'), organizationId: OrganizationId('org-1'), active: true });
  ...
  it('does not mutate the array passed into its constructor', async () => {
    ...
    await repository.save({ id: CustomerId('customer-2'), organizationId: OrganizationId('org-1'), active: true });
