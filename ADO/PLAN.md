...
**Stand:** 24.09.2026, Produktion auf `b635c4a` (Deploy 24.09.: T-031 Startseite hinter Passwort mit Caddy-Rueckweg,
T-074 Webs am Handy; Controller `b635c4a` installiert; Migrationen bis 032). iPhone Stufe 1 mit T-072b im TestFlight-Build
vom 24.09. abgenommen. Naechste Reihenfolge (PO 24.09., D-091, D-093): T-077 ✓, T-076 ✓ und T-062 ✓ auf `main` → **T-078**
(Image-Bau ohne Cache) → **T-079** (Kundensicht) → ein Deploy mit Migrationen 033 und 034 → ein App-Build fuer iPhone und
Android mit Geraeteabnahme → **T-024** → Pilot Monat 1. In Monat 1 zusaetzlich T-075 (Paketgrenze, D-087).
...
| **T-050** | **Pausenautomatik (D-047)** | Der Pausen-Tag verschwindet aus App, Backend und Oberflaeche. Der Abzug kommt als berechnete Schicht dazu, mit Kennzeichnung fuer den Abzug ohne Luecke und eigener CSV-Spalte. **Braucht T-036 und T-048; zusammen mit ihnen waehrend Pilotmonat 1 (D-063).** | — |
...
| **T-078** | **Der Image-Bau haengt nie am Cache (D-094)** | Am 24.09. brach der Image-Lauf fuer `130d115` nach 20 Minuten ab, weil der Export in den GHA-Cache hing. Bau ohne GHA-Cache, eigene Zeitgrenze je Bau-Schritt, Dauer gemessen und im Bericht. Nur `container-image.yml` und ein Satz in `DEPLOY.md`. Vor dem naechsten Deploy. Brief in TASK.md. | 1 |
| **T-079** | **Kundensicht: Was der Kunde sieht, stimmt und ist verstaendlich (D-093)** | Aus dem externen Review vom 24.09.: Erfolgsmeldungen der Verwaltung erscheinen heute rot als Fehler (P1, zwoelf Meldungen); Kalender in App und Web mit Pausenabzug wie der Export (Migration 034, alte App-Staende bekommen byte-gleiche Antworten); Export-Knopf sagt, was er liefert, Formatwahl ohne „v3/v4“; Fachsprache und Ausnahme-Texte in Alltagssprache; Willkommensseite mit naechstem Schritt; Zeiteingabe in der App mit Datum und Uhrzeit getrennt; ehrliche Aussagen auf der Startseite. Nach T-078, vor Deploy und App-Build. Brief nach T-078 in TASK.md. | 3 |
