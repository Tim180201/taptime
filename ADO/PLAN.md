...
**Stand:** 25.09.2026, Produktion auf `6c7007d` (Deploy 25.09.: T-062, T-078, T-079; Migrationen bis 034; Controller `b635c4a`
unveraendert). App-Builds vom 25.09. auf `6c7007d` (iPhone 1.0.0 (3), Android versionCode 10). Geraeteabnahme laeuft; Befund
P1: die Standortleitung bekommt in der App keine Offline-Freigabe. T-080 ✓ auf `main`. Naechste Reihenfolge (PO 25.09.): **T-083**
(Sicherung bleibt kurz) → Deploy → App-Builds → Rest der Geraeteabnahme → **T-024** → Pilot Monat 1. In Monat 1 zusaetzlich T-075 (Paketgrenze, D-087)
...
| **T-081** | **Beschaeftigte nach Standort (Wunsch PO 25.09.)** | Bei eingeschalteten Standorten in „Beschaeftigte“ (App und Web) zuerst den Standort waehlen, dann die Personen; in der App fuer die Standortleitung die Ueberschrift „Mitarbeiter deines Standorts“. Nur Anzeige, keine neue Berechtigung. Pilotmonat 1. | 1 |
| **T-082** | **Pause auf dem Erfassen-Bildschirm (D-096)** | Bei laufender Arbeitszeit ein Knopf „Pause starten“ / „Pause beenden“ mit „Pause laeuft seit …“ direkt auf Erfassen (heute unter „Manuell starten“); Erinnerung nach 45 Minuten bei geoeffneter App und beim naechsten Oeffnen. Gleiches Ereignis wie heute, online und offline. Pilotmonat 1, vor T-050. | 1 |
| **T-083** | **Die Sicherung bleibt kurz, der Waechter passt sich an (Befund 25.09.)** | Stuendliche Sicherungen dauern 10 bis 44 min bei winziger Datenbank: je Stunde ein Basisarchiv und je WAL-Segment ein Borg-Archiv, aufgeraeumt nur sonntags, dazu zwei getrennte Borg-Caches, die jedes Mal neu abgleichen. Taeglich aufraeumen inkl. WAL-Archive nach dem vorhandenen Floor-Verfahren, ein gemeinsamer Cache, `borg compact`; Waechter toleriert die Dauer der letzten Sicherung statt fest zehn Minuten. Nur Betriebsskripte, Tests mit nachgebautem Borg nach dokumentierter Semantik, dann Deploy ohne Konsolenschritt. Vor dem Pilot, nach T-080. | 1 |
