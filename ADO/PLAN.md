...
Naechste Reihenfolge: **T-067** → Deploy → **T-065** → **T-066** → Deploy → Pilot Monat 1;
...
| **T-066** | **Zeit nachtragen und Kommentar (D-067)** | Mitarbeiter traegt im laufenden und im Vormonat nach, zaehlt sofort, markiert „nachgetragen"; Administrator und Standortleitung ohne Grenze mit Grund; Kommentar je Zeiteintrag, Historie erhalten, im Export. Backend, App, Web. Vor dem Pilotstart. | 2 |
| **T-065** | **Abnahme am Geraet 21.09.: Kalender, Offline-Start, Manuell-Knopf** | Monat passt ohne Wischen auf den Bildschirm; Offline-Start in Sekunden statt einer Minute, mit Namen der angemeldeten Person, Verwaltung bleibt ohne Server aus; „Manuell starten" als Knopf auf Erfassen, Seite Manuell entfaellt, Pause bleibt. Nur `apps/mobile`. | 2 |
...
| **T-062** | **Pruefen im eigenen Standort fuer die Standortleitung (D-059, D-064)** | `has_current_time_review_administrator_v1` (012) und `TimeReviewCoordinator.withAdministrator` verlangen heute `administrator`; D-059 sieht Pruefen fuer die Standortleitung vor. Muster ist T-060: eine SQL-Autoritaet mit Standortgrenze (Zielperson im eigenen Standort), Backend gibt die echte Rolle weiter, Sitzung traegt `review_items` dann auch fuer die Standortleitung. Rotnachweis ueber die Standortgrenze und einen zweiten Betrieb, unabhaengiges Review. **Termin: Pilotmonat 1 neben T-048/T-050; rueckt vor, wenn der Pilotbetrieb Standorte einschaltet.** | 3 |
