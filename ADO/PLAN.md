...
**Stand:** 16.09.2026, Reihenfolge aus der vollständigen Anforderungsprüfung gegen den Code
(D-012).
...
| **T-039** | **Das Auslieferungstor prüft die richtige Herkunft, nicht irgendeine** | Das Bündeltor vergleicht die im Bündel gebackene Supabase-Herkunft gegen den Aussteller, dem das Backend tatsächlich vertraut. Erst dann beweist eine grüne Auslieferung, dass sich jemand anmelden kann. **Gegenbeweis Pflicht:** Ein Bündel mit fremder, aber formal gültiger Herkunft muss die Auslieferung zum Abbruch bringen. | — |
| **T-040** | **Die Anmeldung nennt die Ursache** | `SupabaseMemoryAuth.signIn()` liefert einen booleschen Wert; `AdminWebCoordinator.ts:1750` macht daraus in jedem Fall denselben Satz. Mindestens zu unterscheiden: Zugangsdaten abgelehnt, Anmeldedienst nicht erreichbar, Konto nicht bestätigt, Ratenbegrenzung. Kein Auffangzweig mit Bedeutung — vollständige Zuordnung mit `never`-Zweig nach der Regel in `AGENTS.md`. Betrifft die Website und die App gleichermaßen. | — |
...
| **T-048** | **Der Arbeitstag und seine Freigabe — Backend (D-046)** | Der Tag als Begriff, die zwei Stufen, die Sperre, Oeffnen und Ablehnen mit Begruendung, Sperre bei offenem Prueffall. Erst wenn das traegt, lohnt eine Oberflaeche dafuer. **Braucht T-036**, weil ein Arbeitstag eine Tagesgrenze braucht. | — |
...
| **T-036** | **Zeitrichtigkeit (B02, D-056)** | `navigation.ts:94` bildet Monatsgrenzen mit `Date.UTC`; damit beginnt JEDER deutsche Kalendermonat um 01:00 bzw. 02:00 Uhr Berliner Zeit statt um Mitternacht — nicht nur zwei Wechsel im Jahr, wie hier vorher stand (Audit 17.09.). `navigation.test.ts:43` zementiert den Fehler. Dazu das Exportfenster: 31 x 24 h ist kuerzer als ein Oktober in Berlin (31 Tage + 1 Stunde), der Oktober-Export scheitert. Ein bereits vorhandenes `timeZone.ts` im Admin-Web kann Wandzeit in UTC umrechnen und behandelt die doppelte Stunde; es wird heute nur zur Anzeige benutzt, mit der Browser-Zone. **Blockiert T-048, T-049, T-050.** | — |
