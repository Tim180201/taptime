...
| R-026 | A physical procedure that authenticates an Administrator for setup and then a different Employee on the same installation can conflict with the deliberately permanent encrypted offline-store owner binding and block all later capture as an identity mismatch. | Security / Operations | Critical | Reopened for DA5 Product Human V5 only; technical correction published, CI-green and artifact-approved; open only pending a new separately authorized Human Hardware run; historical `DA3-PHYS-01` closure retained | The `2026-08-10` DA5 R4 run reproduced the cross-identity owner conflict after enrollment sign-out, before Tag-B presentation or any scan/lifecycle/queue/time/review mutation, and ended with terminal null cleanup. Published executable `416f1bc8a7803b1ce96d0d767d6b0179ea1ffb6a` / tree `ba9d884fd7eb71b988ecaab9991efbc995141f32` applies the DA3 clean-install precedent through an exact Human-confirmed pre-state, old offline close, exact-package/owned-mapping cleanup, same immutable APK reinstall, fresh offline arm and exact unchanged post-state. Runner reuse remains blocked until identity-matched cleanup succeeds. Exact-Head CI `31430310571`, attempt 1, passed 12/12 without retry; final OS-sandboxed Exact-Head/Artifact Review is `APPROVED` with zero open P0–P3. No Hardware, ADB, installation or Operator run occurred after publication. The prior DA3 accepted evidence publication `7cb510a` and run `29996799069` 12/12 remain historical and unchanged. STOP before Hardware; a new exact Human authority is required. |
