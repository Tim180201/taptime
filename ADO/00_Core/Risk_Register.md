...
| R-026 | A physical procedure that authenticates an Administrator for setup and then a different Employee on the same installation can conflict with the deliberately permanent encrypted offline-store owner binding and block all later capture as an identity mismatch. | Security / Operations | Critical | Reopened for DA5; package-clear/ready Round-1 P2 corrected and locally green; independent re-review plus later separate Hardware authority required | The consumed run stopped before Tag-B presentation or scan/lifecycle/queue/time/review mutation. On exact baseline `aa9ad8c1d5cb74bc68b7702a41d9b154906921d0` / tree `3f1e0f4cd812fc37153cf2de5a0ce6f7506a2362`, the Human-authorized candidate adds only one owning-transaction, serial/package-exact `pm clear --user 0 com.tim180201.mobile.synthetic` after same-APK replacement and exact `Success\n`/empty-stderr receipt. Round-1 review returned `CHANGES REQUIRED` for exactly one P2 and no other finding: the final state proof could trail APK provenance/signer/digest and process-null was not rebound at the immediate offline arm. The correction reattests current device, exact package, two owned mappings and process-null after the complete digest, then proves process-null again read-only at the Employee-only arm without App start or mapping mutation. It stops `employee-prepared`; exact Human `Bereit zum Scannen` plus unchanged server/Tag/session-lifecycle/queue/time/review proof alone makes `matched`. Tag/Gate/presentation stays blocked beforehand and all ambiguous/unavailable/order/concurrency paths fail closed. Focused Mobile 120/120, Synthetic 107/107, ProductStart 5/5, one final full required-reachability Synthetic 345 passed/19 expected skips, Typechecks/membership and build/syntax passed. Product/APK/schema/dependency/workflow/backup semantics and cleanup are unchanged. No Hardware/ADB/install ran; re-review pending; STOP before Hardware. |
