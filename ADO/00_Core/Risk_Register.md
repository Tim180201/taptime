...
and terminal cleanup is null; the local correction candidate is not Hardware-ready.** The
...
The candidate inserts a Human-confirmed, exact-state, same-APK clean installation boundary between
...
ADO-only correction now separates carried pre-Round-1 Mobile evidence from fresh post-Round-1
Synthetic evidence and performs no test/build rerun. Independent re-review, publication,
Exact-Head CI, fresh artifact review and new Human authority are required before Hardware.
...
| R-026 | A physical procedure that authenticates an Administrator for setup and then a different Employee on the same installation can conflict with the deliberately permanent encrypted offline-store owner binding and block all later capture as an identity mismatch. | Security / Operations | Critical | Reopened for DA5 Product Human V5 only; historical `DA3-PHYS-01` closure retained; correction candidate not Hardware-ready | The `2026-08-10` DA5 R4 run reproduced the cross-identity owner conflict after enrollment sign-out, before Tag-B presentation or any scan/lifecycle/queue/time/review mutation, and ended with terminal null cleanup. The local correction applies the DA3 clean-install precedent through an exact Human-confirmed pre-state, old offline close, exact-package/owned-mapping cleanup, same immutable APK reinstall, fresh offline arm and exact unchanged post-state. Runner reuse remains blocked until identity-matched cleanup succeeds. Review Round 2's sole P3 concerned only carried/fresh evidence labeling and is corrected ADO-only without rerun; independent re-review, publication, Exact-Head CI, fresh artifact review and new one-time Human authority remain required. The prior DA3 accepted evidence publication `7cb510a` and run `29996799069` 12/12 remain historical and unchanged. |
