...
| R-026 | A physical procedure that authenticates an Administrator for setup and then a different Employee on the same installation can conflict with the deliberately permanent encrypted offline-store owner binding and block all later capture as an identity mismatch. | Security / Operations | Critical | Reopened for DA5; final-review Round-1 P2 corrected and fresh local R3 green; independent re-review plus later separate Hardware authority required | The consumed run stopped before Tag-B presentation or scan/lifecycle/queue/time/review mutation. Published baseline `5fa6389aad785007b0322a38cfd81a17816c7eab` / tree `2387276f4969e9ca16cbb31e03a4ad50949bbaba` contains the Human-authorized exact-package clear and Employee-ready boundary. Formal final Exact-Head/Artifact Review Round 1 returned `CHANGES REQUIRED` with exactly one P2 and no other finding: the immediate Employee arm used Toybox `ps -A -o NAME`, permitting long Product primary/secondary process-name truncation. The focused uncommitted correction uses normative `ps -A -w -o NAME:4` while retaining exact exit/stdout/header/row parsing, the failure latch, no App start and no mapping mutation. Focused Controller 60/60, required-reachability ProductStart 5/5, tests-inclusive 573-file Typecheck membership, build/syntax/map invariants and exactly one final full required-reachability Synthetic run at 345 passed/19 expected skips are green. Product/Mobile/APK/schema/dependency/workflow/backup semantics and cleanup are unchanged. No Hardware/ADB/install ran; independent re-review pending; STOP before Hardware. |
