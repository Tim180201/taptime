...
## Current DA5 Human-order-deviation correction-2 override

Marker `DA5-V5-ABORT-BEFORE-PREVIEW2-CORRECTION2-2026-08-13` supersedes correction 1's
inaccurate child-TMP relationship, its overbroad no-entry wording and every conflicting generic
task-state, immediate-cleanup, queue or clean-working-tree assertion. Terminal classification
remains **`FAIL_CLOSED / HUMAN ORDER DEVIATION BEFORE PREVIEW 2; LATER CURRENT STATE CLEAN`**,
never Product PASS and no Product-defect finding. Authority `E` is consumed; no retry, resume,
relogin or replacement run is authorized.
...
R-026/R-034 remain open physical Product risks without a new Product finding. AVS scope is R0/V0
only; no test, build, Typecheck, CI, ADB, Hardware, commit or push is run or claimed.

## Preserved DA5 Human-order-deviation correction-1 override

Preserved DA5 Human-order-deviation correction-1 override (`2026-08-13`), marker
`DA5-V5-ABORT-BEFORE-PREVIEW2-CORRECTION1-2026-08-13`: terminal classification is
...
The immutable original `0555` root
`/Users/timbartz/Dokumente/GitHub/taptime-local-evidence/da5-v5/hardware-42b330a-abort-before-preview2-20260813`
...
cleanup; immediate complete cleanup is unverified and cannot be reconstructed.
...
empty, standard profile `MATCH`, Operator absent and a historical tracked/staged-clean report;
then `current_guard_cleanup=MATCH` and the listed Guard/PostgreSQL/owned-port checks. Correction 2
supersedes correction 1's child-base and no-entry scope. This proves later current state only. The
schema has no queue field, the
queue checkpoint was not reached and no machine queue Evidence exists; queue is unobserved and
unclaimed. This correction supersedes every conflicting generic task-state, immediate-cleanup or
queue assertion in the preserved predecessor paragraph below.

Preserved predecessor DA5 Human-order-deviation override (`2026-08-13`), marker
`DA5-V5-ABORT-BEFORE-PREVIEW2-CLOSURE-2026-08-13`: on exact `E`
`42b330a1ea700169d7adcd1c3bf54e3dfb868d0a` / tree
`21aad9fa3e0dd7de1d87c66fdaaba8ee0cdc6a92`, parent `C`
`33a1d70c06b0275c59be20bf9d5afc4c8af44767`, fresh preflight, child-environment,
Operator/device/install and Administrator Credential/login boundaries matched. Initial digital
state was zero; Human setup PASS plus machine reattestation established one active Tag-A and
Customer-A assignment and one setup receipt with zero lifecycle and NFC/manual/time state; queue
was not proved.

The Human then reported an accidental sign-out before mandatory Admin Setup Preview 2. It is an
order deviation, not a Product defect. No relogin was permitted; ordinary-idle `abort` occurred
exactly once, emitted `da5_v5_aborted` and exited 1. Correction 1 binds only later current state,
not immediate cleanup. Outcome is **`FAIL_CLOSED / HUMAN ORDER DEVIATION BEFORE PREVIEW 2; LATER
CURRENT STATE CLEAN`**; authority `E` is consumed and no retry/resume/relogin/replacement is
authorized. Preview 2, Employee, Tags B/X, Gates B–F and Accessibility were not reached; no
lifecycle/time event was observed at the reached machine checkpoints, and queue state/event is
unobserved and unclaimed. Thus R-026/R-034 remain open physical Product risks without a new
Product finding or a new executable correction decision.

Immutable Evidence root
`/Users/timbartz/Dokumente/GitHub/taptime-local-evidence/da5-v5/hardware-42b330a-abort-before-preview2-20260813`
is `0555`; receipt 3,825 bytes / `0444` /
`98e7278983dce827133cddcdfb3cb1617b6b072179ad39e93f6ea44ab3221f94`; manifest 1,827 bytes /
`0444` / `db24eb313181a64f488becf552c0fd9d70583e375a7f79d17c5ca357afae3813`.
Human reports are explicitly separate from machine receipts and later current-state checks; the
original generic task-state and queue claims are superseded. This is an AVS R0/V0 eight-path ADO
synchronization only; no tests, CI or Hardware action is attributed to it.
...
run.** The
enrollment-only Administrator identity remained the permanent encrypted offline-store owner, so
the later Employee identity could not acquire the same installation. The Employee path never
reached ready, Tag B was not presented, a cold reopen showed protected pending state without a
scan, and no scan/lifecycle/queue/time/review mutation occurred. This is the DA5 recurrence of the
already-closed `DA3-PHYS-01` procedure risk, not a reopening of that historical accepted result.
The published correction inserts a Human-confirmed, exact-state, same-APK clean installation boundary between
enrollment sign-out and Employee readiness; no broad reset or Product/APK change is allowed.
Review Round 2 returned `CHANGES REQUIRED` only for one P3 evidence-provenance ambiguity; the
ADO-only correction separates carried pre-Round-1 Mobile evidence from fresh post-Round-1
Synthetic evidence and performed no test/build rerun. Executable
`416f1bc8a7803b1ce96d0d767d6b0179ea1ffb6a`, tree
`ba9d884fd7eb71b988ecaab9991efbc995141f32`, publishes the exact 13-path, 81,331-byte delta at
SHA-256 `4d586d2ad5318f288e611393554b2fbc25731c95fd35e4c1fc1376d30737c70c`.
Exact-Head CI `31430310571`, attempt 1, passed 12/12 without retry. Fresh read-only runtime
`/Users/timbartz/Dokumente/GitHub/taptime-local-artifacts/da5-v5-product-operator/416f1bc8-8d4981e5`
binds manifest/bundle/map at mode `0444`; final OS-sandboxed Exact-Head/Artifact Review rejected
the adversarial write with `EPERM`, proved candidate/artifacts unchanged and returned `APPROVED`
with zero open P0–P3. No Hardware, ADB, installation or Operator run followed. This R0 closure
sync performs no execution gate. **STOP before Hardware; DO NOT INSTALL / DO NOT START without
new exact Human authority.**
...
| R-026 | A physical procedure that authenticates an Administrator for setup and then a different Employee on the same installation can conflict with the deliberately permanent encrypted offline-store owner binding and block all later capture as an identity mismatch. | Security / Operations | Critical | Technically mitigated; open only pending a new separate exact Human Hardware authorization/run | Published executable `22fe85d540c8949f179b96589ed493f0211002db` / tree `7514edfe90da11a3288fec0df872fb7010238c0b`, direct parent `5fa6389aad785007b0322a38cfd81a17816c7eab` / tree `2387276f4969e9ca16cbb31e03a4ad50949bbaba`, closes the package-only clear and Employee-ready technical boundary. Exact nine-path delta: 45,172 bytes / SHA-256 `2346cccc6e3727f39f0f09470fc5e2a66cd8dbba8f20d9aa95da2c1d206fc592`; prepublication provenance: three code/test paths 5,037 bytes / SHA-256 `61c56a2afcf9a986f69fe67f28de2fda037d1e1cac942407b943a0b3b5fda2ea`, six ADO paths 40,135 bytes / SHA-256 `51da9ddc31788e020f85ce3b54aee8b01568e9d846f0cdace6c4fdc9a98c337f`. Round 1's sole P2 was Toybox process-name truncation; normative `ps -A -w -o NAME:4` now retains long primary/secondary names with exact parser/fail-closed/no-start/no-mapping-mutation semantics. Exact-head CI `31630253237`, attempt 1, passed 12/12 without retry. Fresh read-only runtime `/Users/timbartz/Dokumente/GitHub/taptime-local-artifacts/da5-v5-product-operator/22fe85d5-2a48d52d` binds mode-`0444` manifest 10,726 bytes / SHA-256 `2e24bb5c5d39089ef871d9fd986a67532d51f71df3a6e1652b1b1e232bf6e1fe`, mode-`0444` bundle 930,095 bytes / SHA-256 `2a48d52d204ef8cfba73d9a789de3ef50ecefa29e32c51953bc75b9d48d023a6`, and mode-`0444` map 1,728,717 bytes / SHA-256 `a51b9cfa0dfa883bf5d091789051ec16c85529afbecd53b37017940fa16a70d2`, version 3/no `sourceRoot`/90-90-90; the manifest's generation-time pending field is historical. Final strict OS-sandboxed Exact-Head/Artifact Review returned `APPROVED`, zero open P0–P3, with unchanged identities and local immutable-manifest/exact-ref CI proof under network denial. The prior consumed Hardware run and abort/null cleanup remain non-reusable. Product APK, Guard, device, Tag, path and Credential constraints remain unchanged; production/deployment/distribution unauthorized. STOP before Hardware. |
