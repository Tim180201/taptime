...
PREPUBLICATION CANDIDATE MAKES NO SELF-CLAIM OF ITS OWN PUBLICATION, REMOTE BINDING, REPLACEMENT
V4 OR FINAL REVIEW; UNCHANGED RETRY FORBIDDEN; ATTEMPT 13 NOT EXECUTED / DO NOT EXECUTE;
HARDWARE/HUMAN/PRODUCT V5 DO NOT START**. The failed job used a fixed July UTC window that
excluded its current-date lifecycle record. The locally verified correction instead binds the
exact server-verified record and uses one deterministic contract-bounded UTC window for query and
export. Any later publication, remote binding, replacement-V4 or final-review outcome must be
bound after it actually occurs in a downstream closure synchronization. This override supersedes
older Attempt-10/11/12/13 candidate-state text below without changing its historical evidence.
