...
## 10. Implemented Reality (Development Sprint 001–004 & 006–019)
...
### 10.79 Repository Write Extensions Implementation (DT-020, DT-021, DT-022)

Development Sprint 015 implements the fourth slice of FB-002/TS-002 (Organization Management Foundation), the first to bundle three Development Tasks into one sprint: `CustomerRepository.save(customer: Customer): void` (`packages/core/src/ports/CustomerRepository.ts`), `NfcTagRepository.register(nfcTag: NfcTag): void` (`packages/core/src/ports/NfcTagRepository.ts`), and `NfcAssignmentRepository.save(nfcAssignment: NfcAssignment): void` (`packages/core/src/ports/NfcAssignmentRepository.ts`) — each an additive method on a port that previously exposed only one read method (`findById`/`findByPayload`/`findActiveByTagId` respectively, each confirmed unchanged, byte-for-byte). `InMemoryCustomerRepository`, `InMemoryNfcTagRepository`, and `InMemoryNfcAssignmentRepository` each implement their new method as a single `.push(...)` onto an internally-held array, and each constructor was upgraded from a bare-reference assignment to the defensive-copy pattern (`this.x = [...x]`) `InMemoryOrganizationRepository`/`InMemoryMembershipRepository` already use. `CustomerCreated`, `NfcTagRegistered`, and `NfcTagAssigned` (`packages/core/src/domain/events/`) were added, each a `{ type, <payload> }` interface plus a lowercase constructor function, following `WorkEventCreated`/`OrganizationCreated`/`MembershipGranted`'s idiom exactly. Commit `7db5ade`.

### 10.80 Why Three Tasks Were Bundled, and Why That Was Independently Re-Verifiable

`Development_Sprint_015_Plan.md` Section 4 justified bundling DT-020/DT-021/DT-022 on the grounds that the three tasks share no file and no mutual dependency. This closure independently re-verified that claim rather than accepting it at face value: `git diff --stat` against the pre-Sprint-015 commit (`874ecaf`) shows the three tasks' changes cleanly separable into three disjoint groups (Customer files, NfcTag files, NfcAssignment files), and each of DT-020/DT-021/DT-022's own Acceptance Criteria (Section 10.79 above) was checked against actual source with no reference required to either of the other two tasks. This is the practical test for whether a bundled Development Sprint was, in fact, "smallest safe" rather than merely convenient — see EP-008 Ch01 Section 10.15 for the fuller reasoning.

### 10.81 Testing Strategy for Repository Write Extensions

Three new dedicated test files were added — `InMemoryCustomerRepository.test.ts` (5 tests), `InMemoryNfcTagRepository.test.ts` (5 tests), `InMemoryNfcAssignmentRepository.test.ts` (6 tests) — the first-ever dedicated unit test coverage for these three repositories; previously they were exercised only indirectly through `AssignmentResolver.test.ts`/`AssignmentValidator.test.ts` fixtures. Each new file covers: the pre-existing not-found (or not-active) case continuing to behave correctly; a round-trip proof (write then read returns what was written); a "does not find under a different key" case; constructor-seeded lookups continuing to work; and an explicit proof that the constructor's input array is not mutated by a subsequent write. `npm run test --workspace=@taptime/core` reports 39 test files and 197 tests passing (181 pre-existing + 16 new); `npm run typecheck --workspace=@taptime/core` is clean. `AssignmentResolver.test.ts` (5 tests) and `AssignmentValidator.test.ts` (5 tests) were both re-run and confirmed to pass with zero modification, verified additionally by an empty `git diff` against both files and both their test files. `npm run typecheck --workspace=apps/mobile` is clean and `npm run test --workspace=apps/mobile` reports 10/10 passing, confirming `apps/mobile` was not affected.

### 10.82 Common Implementation Pitfalls (Repository Write Extensions)

- Wiring any of the three new write methods into `AssignmentResolver`, `AssignmentValidator`, `MembershipService`, `OrganizationManagementService`, or any Application Service "since it's now available" — Out of Scope for DT-020/DT-021/DT-022; no caller was introduced, confirmed by direct search.
- Adding an authorization check, an Organization-existence check, or a same-Organization cross-check inside any of the three new write methods — each Decision's precondition (FB-002 Decisions 3/4/5) is explicitly DT-023/024/025's responsibility, not this sprint's; the write methods perform no such check.
- Resolving tag reassignment/history semantics, tag payload collision semantics, or "one active assignment per tag" inside `InMemoryNfcAssignmentRepository.save` — all three remain named FB-002 Open Questions; the save method accepts and stores whatever `NfcAssignment` it is given, with no uniqueness enforcement of its own, confirmed by direct inspection of the method body (a single `.push(...)`).
- Modifying `AssignmentResolver.ts`, `AssignmentValidator.ts`, or either of their test files while adding nearby test coverage — confirmed to have zero diff against the pre-Sprint-015 commit; this was checked directly, not assumed.
- Starting DT-023 (`OrganizationAdministrationService` Customer registration), DT-024, DT-025, or DT-026 "while already in this area" — each is a separate, later Development Task; DT-023 in particular depends on both this sprint's DT-020 and the earlier DT-019, and creates a shared class DT-024/DT-025 will also extend, which is exactly why it was not bundled into this sprint (`Development_Sprint_015_Plan.md` Section 5). Confirmed by direct search finding no `OrganizationAdministrationService` file anywhere in the repository.

### 10.84 Organization Administration: Customer Registration Implementation (DT-023)

Development Sprint 016 implements the fifth slice of FB-002/TS-002 (Organization Management Foundation), and the first Application Service orchestration step: `OrganizationAdministrationService` (`packages/core/src/application/OrganizationAdministrationService.ts`), created for the first time, with exactly one method, `createCustomer(membership: Membership | null, organizationId: OrganizationId): CreateCustomerResult`. The constructor takes `MembershipAuthorizationValidator`, `CustomerRepository`, and an injectable `newCustomerId: () => CustomerId = () => CustomerId(generateId())`, following `OrganizationManagementService`'s/`MembershipService`'s established constructor pattern exactly. `createCustomer(...)` calls `this.membershipAuthorizationValidator.authorize(membership, organizationId)` first; on rejection, it returns that result unchanged, with no `Customer` construction and no `CustomerRepository.save` call; on acceptance, it constructs a `Customer` (`id` from `newCustomerId()`, `organizationId` from `authorizationResult.membership.organizationId` — the accepted Membership's own Organization, per TS-002 Decision 3, not the raw `organizationId` parameter — and `active: true`), calls `this.customerRepository.save(customer)`, and returns `{ status: 'accepted', customer, event: customerCreated(customer) }`. Commit `5f95573`.

### 10.85 `CreateCustomerResult`: Reusing an Existing Rejection Shape, Not Inventing a New One

`CreateCustomerResult` is defined as `{ status: 'accepted'; customer: Customer; event: CustomerCreated } | RejectedMembershipAuthorizationResult`, co-located in the same file as `OrganizationAdministrationService`. This design was reasoned from repository evidence, not assumed: FB-002 Decision 3's own product-level prose ("Result: `CustomerCreated`, or `AdministrativeActionRejected`") could be read as naming a new rejection type, but TS-002's Sequence Diagram 3 is the more precise, implementation-facing evidence, and it names the exact three rejection reasons `MembershipAuthorizationResult` already defines ("accepted | membership_not_found | membership_lacks_administrator_role | cross_organization_access") — confirming reuse, not invention, is correct. Only the minimum new accepted-branch shape was added (the created `Customer` and its `CustomerCreated` event), since `MembershipAuthorizationResult`'s own accepted variant carries a `Membership`, not a `Customer`.

### 10.86 Testing Strategy for Organization Administration: Customer Registration

`OrganizationAdministrationService.test.ts` (6 tests): the accepted path (an Administrator Membership whose `organizationId` matches the target Organization results in the expected `Customer`/`CustomerCreated` shape, verified by `toEqual`); a dedicated spy-based test confirming `CustomerRepository.save` is called exactly once with the constructed `Customer` on the accepted path; each of the three required rejection branches independently (`membership_not_found` for a `null` Membership, `membership_lacks_administrator_role` for an Employee Membership, `cross_organization_access` for an Administrator Membership of a different Organization), each with its own no-write assertion; and a sixth test proving, via a single consolidated spy assertion, that `CustomerRepository.save` is never called across all three rejection scenarios. All 6 tests are new; `npm run test --workspace=@taptime/core` reports 40 test files and 203 tests passing (197 pre-existing + 6 new); `npm run typecheck --workspace=@taptime/core` is clean. `MembershipAuthorizationValidator.test.ts`, `InMemoryCustomerRepository.test.ts`, `AssignmentResolver.test.ts`, and `AssignmentValidator.test.ts` were all re-run and confirmed to pass with zero modification, verified additionally by an empty `git diff` against all four files and their corresponding source files. No composition-level or `apps/mobile` test exists for this capability — no UI, CLI, or mobile entry point calls `OrganizationAdministrationService`, per DT-023's own Out of Scope.

### 10.87 Common Implementation Pitfalls (Organization Administration: Customer Registration)

- Implementing `registerNfcTag(...)` or `assignNfcTag(...)` alongside `createCustomer(...)` "since the class already exists now" — DT-024/DT-025's responsibility, not this task's; confirmed absent by direct `grep` finding no occurrence of either method name anywhere in `packages/core/src`.
- Inventing a new `AdministrativeActionRejected` result type instead of reusing `MembershipAuthorizationResult`'s existing rejection shape — Section 10.85 traces this directly to TS-002 Sequence Diagram 3's own evidence; the rejection branch reuses `RejectedMembershipAuthorizationResult` verbatim.
- Modifying `MembershipAuthorizationValidator` to add a Customer-specific parameter or check — DT-023's Implementation Boundary states the validator is "consumed, not modified"; `createCustomer(...)` calls `authorize(membership, organizationId)` exactly as it exists today.
- Sourcing `Customer.organizationId` from the raw `organizationId` parameter instead of the accepted Membership's own `organizationId` — both are guaranteed equal in the accepted case by `authorize`'s own cross-Organization check, but the implementation deliberately sources from `authorizationResult.membership.organizationId`, the more faithful reading of "Customer.organizationId comes from the requesting Membership's Organization" (TS-002 Decision 3).
- Adding Customer field validation beyond `Customer`'s existing shape (`id`, `organizationId`, `active`) "since this is the first real creation flow" — Out of Scope, per DT-023's own Out of Scope.
- Wiring `OrganizationAdministrationService` into a UI, CLI, or composition root "to make it usable" — Out of Scope, per DT-023's own Out of Scope; no caller was introduced for `createCustomer(...)` by this sprint.
- Modifying `MembershipAuthorizationValidator.test.ts`, `InMemoryCustomerRepository.test.ts`, `AssignmentResolver.test.ts`, or `AssignmentValidator.test.ts` while adding nearby test coverage — confirmed to have zero diff against the pre-Sprint-016 commit; this was checked directly, not assumed.

### 10.89 Organization Administration: NFC Tag Registration Implementation (DT-024)

Development Sprint 017 implements the sixth slice of FB-002/TS-002 (Organization Management Foundation), and the second method on `OrganizationAdministrationService` (`packages/core/src/application/OrganizationAdministrationService.ts`): `registerNfcTag(membership: Membership | null, organizationId: OrganizationId, payload: NfcPayload): RegisterNfcTagResult`. The constructor gains a third required dependency, `NfcTagRepository`, placed before both existing defaulted id generators (`membershipAuthorizationValidator, customerRepository, nfcTagRepository, newCustomerId = default, newNfcTagId = default`). `registerNfcTag(...)` calls `this.membershipAuthorizationValidator.authorize(membership, organizationId)` first; on rejection, it returns that result unchanged, with no `NfcTag` construction and no `NfcTagRepository.register` call; on acceptance, it constructs an `NfcTag` (`id` from `newNfcTagId()`, `organizationId` from `authorizationResult.membership.organizationId` — mirroring `createCustomer(...)`'s own precedent, per TS-002 Sequence Diagram 4, not the raw `organizationId` parameter — and `payload` taken from the method's own already-normalized `NfcPayload` argument, unchanged), calls `this.nfcTagRepository.register(nfcTag)`, and returns `{ status: 'accepted', nfcTag, event: nfcTagRegistered(nfcTag) }`. `createCustomer(...)`'s own method body is byte-for-byte unchanged from its DT-023 form. Commit `eee9151`.

### 10.90 `RegisterNfcTagResult` and the Constructor-Arity Regression: What Happened and How It Was Fixed

`RegisterNfcTagResult` is defined as `{ status: 'accepted'; nfcTag: NfcTag; event: NfcTagRegistered } | RejectedMembershipAuthorizationResult`, co-located in the same file as `CreateCustomerResult`, reusing `RejectedMembershipAuthorizationResult` verbatim for the same evidence-based reason Section 10.85 records for DT-023, now traced to TS-002 Sequence Diagram 4 rather than Sequence Diagram 3.

This sprint's implementation reached its final, closeable state only after a disclosed correction, recorded here as genuine repository history rather than omitted (EP-008 Ch01 §5.4, Reality Has Priority Over Architecture). The first implementation pass added `nfcTagRepository` as a constructor parameter positioned *after* the already-existing defaulted `newCustomerId` parameter — a signature TypeScript accepts without a declaration-site error, but one that silently changes the constructor's required positional arity from 3 to 5, breaking every pre-existing `createCustomer` test that still called the constructor with only 3 positional arguments. This went undetected by `npm run typecheck --workspace=@taptime/core` because `packages/core/tsconfig.json`'s `"include": ["src"]` does not cover `tests/`, and undetected by `npm run test --workspace=@taptime/core` because Vitest's esbuild-based transpilation does not perform real type-checking — the six affected tests reported passing (false-green) despite being, under a real compiler, invalid calls. An independent Review Agent reproduced this directly, using a temporary tests-inclusive tsconfig (`"include": ["src", "tests"]`) and `tsc --noEmit`, surfacing exactly six `TS2554: Expected 4-5 arguments, but got 3.` errors. The corrective Development Agent Prompt specified reordering the constructor so all three required dependencies precede both defaulted id generators — matching `OrganizationManagementService`'s/`MembershipService`'s existing idiom, deliberately not an options-object redesign, since the reorder both fixes the regression and anticipates DT-025's future third required repository dependency without a larger refactor — plus updating the six affected `createCustomer` test call sites to pass a throwaway `InMemoryNfcTagRepository` as the new third argument, with no assertion changed. Commit `eee9151` contains this corrected constructor; this was independently re-confirmed during Development Sprint 017's governance closure by repeating the same tests-inclusive `tsc --noEmit` technique and finding zero errors (`Development_Sprint_017_Closure.md` Section 6).

### 10.91 Testing Strategy for Organization Administration: NFC Tag Registration

`OrganizationAdministrationService.test.ts` (13 tests total: 6 pre-existing `createCustomer` tests, unmodified in assertion, with their constructor call sites updated for the new required `NfcTagRepository` argument; 7 new `registerNfcTag` tests): the accepted path (an Administrator Membership whose `organizationId` matches the target Organization results in the expected `NfcTag`/`NfcTagRegistered` shape, verified by `toEqual`); a dedicated spy-based test confirming `NfcTagRepository.register` is called exactly once with the constructed `NfcTag` on the accepted path; each of the three required rejection branches independently (`membership_not_found`, `membership_lacks_administrator_role`, `cross_organization_access`), each with its own no-write assertion; a consolidated spy assertion proving `NfcTagRepository.register` is never called across all three rejection scenarios; and a deterministic-`NfcTagId`-generation test using an injected fixed generator. `npm run test --workspace=@taptime/core` reports 40 test files and 210 tests passing (203 pre-existing + 7 new); `npm run typecheck --workspace=@taptime/core` is clean. A supplementary, out-of-band tests-inclusive `tsc --noEmit` check (Section 10.90) independently confirms zero type errors remain in this test file specifically. `MembershipAuthorizationValidator.test.ts`, `InMemoryCustomerRepository.test.ts`, `InMemoryNfcTagRepository.test.ts`, `AssignmentResolver.test.ts`, and `AssignmentValidator.test.ts` were all re-run and confirmed to pass with zero modification, verified additionally by an empty `git diff` against all five files and their corresponding source files. No composition-level or `apps/mobile` test exists for this capability — no UI, CLI, or mobile entry point calls `OrganizationAdministrationService`, per DT-024's own Out of Scope.

### 10.92 Common Implementation Pitfalls (Organization Administration: NFC Tag Registration)

- **Adding a new required constructor parameter after an existing defaulted parameter** — the actual defect this sprint produced and corrected (Section 10.90); TypeScript accepts this at the declaration site with no error, but it silently breaks existing positional call sites, and is invisible to both `npm run typecheck` (does not cover `tests/`, per `packages/core/tsconfig.json`'s `"include": ["src"]`) and `npm run test` (Vitest's esbuild transpilation does not type-check). The fix: all required dependencies must precede all defaulted parameters, contiguously, in a class's constructor — verified in this sprint's case using a temporary tests-inclusive `tsc --noEmit` run, not assumed from the reorder alone.
- Implementing `assignNfcTag(...)` alongside `registerNfcTag(...)` "since the class already has three real dependencies wired now" — DT-025's responsibility, not this task's; confirmed absent by direct `grep` finding no occurrence of `assignNfcTag` anywhere in `packages/core/src`.
- Inventing a new rejection result type instead of reusing `MembershipAuthorizationResult`'s existing shape — Section 10.90 traces this directly to TS-002 Sequence Diagram 4's own evidence, the same reasoning DT-023 already established for Sequence Diagram 3.
- Sourcing `NfcTag.organizationId` from the raw `organizationId` parameter instead of the accepted Membership's own `organizationId` — the implementation deliberately sources from `authorizationResult.membership.organizationId`, mirroring `createCustomer(...)`'s own precedent (Section 10.85).
- Re-normalizing or re-validating the `NfcPayload` argument inside `registerNfcTag(...)` — Out of Scope, per DT-024's own Implementation Boundary; the method accepts an already-normalized `NfcPayload` (a branded type only constructible via `createNfcPayload`) and passes it through unchanged.
- Wiring `OrganizationAdministrationService.registerNfcTag(...)` into a UI, CLI, or composition root "to make it usable" — Out of Scope, per DT-024's own Out of Scope; no caller was introduced for `registerNfcTag(...)` by this sprint.
- Modifying `MembershipAuthorizationValidator.test.ts`, `InMemoryCustomerRepository.test.ts`, `InMemoryNfcTagRepository.test.ts`, `AssignmentResolver.test.ts`, or `AssignmentValidator.test.ts` while adding nearby test coverage — confirmed to have zero diff against the pre-Sprint-017 commit; this was checked directly, not assumed.

### 10.94 Organization Administration: NFC Tag Assignment Implementation (DT-025)

Development Sprint 018 implements the seventh slice of FB-002/TS-002 (Organization Management Foundation), and the third and final method on `OrganizationAdministrationService` (`packages/core/src/application/OrganizationAdministrationService.ts`): `assignNfcTag(membership: Membership | null, organizationId: OrganizationId, nfcTag: NfcTag, target: AssignmentTarget): AssignNfcTagResult`. The constructor gains a fourth required dependency, `NfcAssignmentRepository`, placed before all three defaulted id generators (`membershipAuthorizationValidator, customerRepository, nfcTagRepository, nfcAssignmentRepository, newCustomerId = default, newNfcTagId = default, newNfcAssignmentId = default`). `assignNfcTag(...)` calls `this.membershipAuthorizationValidator.authorize(membership, organizationId)` first; on rejection, it returns that result unchanged, performing no further check and no write. On acceptance, it performs two same-Organization checks before any write: it compares the supplied `NfcTag.organizationId` directly against `authorizationResult.membership.organizationId` (no repository call — the `NfcTag` is accepted as an already-resolved value, mirroring `registerNfcTag(...)`'s own already-normalized-`NfcPayload` precedent), and it resolves the supplied `AssignmentTarget`'s owning `Customer` via `this.customerRepository.findById(target.targetId)` and compares its `organizationId` the same way, treating a `null` (not-found) result identically to a mismatch. Either check failing returns `{ status: 'rejected', reason: 'cross_organization_access' }` with no write. Only if both checks pass does it construct an `NfcAssignment` (`id` from `newNfcAssignmentId()`, `organizationId` from the accepted Membership, `nfcTagId` from `nfcTag.id`, `target` unchanged, `active: true`), call `this.nfcAssignmentRepository.save(nfcAssignment)`, and return `{ status: 'accepted', nfcAssignment, event: nfcTagAssigned(nfcAssignment) }`. `createCustomer(...)`'s and `registerNfcTag(...)`'s own method bodies are byte-for-byte unchanged from their DT-023/DT-024 forms. Commit `2c9cdab`.

### 10.95 `AssignNfcTagResult`, the Missing-Target Planning Finding, and Constructor-Arity Discipline Applied Cleanly

`AssignNfcTagResult` is defined as `{ status: 'accepted'; nfcAssignment: NfcAssignment; event: NfcTagAssigned } | RejectedMembershipAuthorizationResult`, co-located in the same file as `CreateCustomerResult`/`RegisterNfcTagResult`, reusing `RejectedMembershipAuthorizationResult` verbatim for the same evidence-based reason Sections 10.85/10.90 record for DT-023/DT-024, now traced to TS-002 Sequence Diagram 5.

This design required resolving one genuine ambiguity repository evidence did not settle on its own, disclosed as a planning finding rather than decided silently (`Development_Sprint_018_Plan.md` Section 8): `AssignmentTarget` carries no `organizationId` of its own, so verifying its ownership requires resolving the underlying `Customer` via `CustomerRepository.findById(...)` — a call that can return `null` if the target `Customer` does not exist. `MembershipAuthorizationResult`'s three rejection reasons contain no "not found" category, and adding one would modify a component this task must not touch. FB-002 Decision 5's own Result line names only two outcomes — `NfcTagAssigned`, or `CrossOrganizationAccessRejected` — at the same precision level Decisions 3/4 used for `createCustomer(...)`/`registerNfcTag(...)`. The resolution adopted, and confirmed present in the committed code (`resolvedCustomer === null || resolvedCustomer.organizationId !== authorizationResult.membership.organizationId`, both producing the identical `cross_organization_access` rejection): a target `Customer` that does not exist cannot be verified as belonging to the requesting Membership's Organization, so "not found" and "found but wrong Organization" are treated identically, requiring no new type or reason. A dedicated test (Section 10.96) exists specifically to prove this resolution is implemented as decided, not merely asserted in prose.

This sprint's constructor extension — the fourth required dependency added to this class across four consecutive sprints — is the first of the three method-adding sprints (DT-023, DT-024, DT-025) to require no post-implementation correction. Unlike DT-024's own disclosed constructor-arity regression (Section 10.90), `Development_Sprint_018_Plan.md` specified the exact constructor signature verbatim rather than describing the change in prose, and the committed code matches it exactly: all four required dependencies precede all three defaulted parameters, confirmed both by direct inspection and by an out-of-band tests-inclusive `tsc --noEmit` run showing zero `OrganizationAdministrationService`-related errors (`Development_Sprint_018_Closure.md` Section 6) — the same reproduction technique that originally caught DT-024's regression, here confirming its absence.

### 10.96 Testing Strategy for Organization Administration: NFC Tag Assignment

`OrganizationAdministrationService.test.ts` (22 tests total: 13 pre-existing `createCustomer`/`registerNfcTag` tests, unmodified in assertion, with their constructor call sites updated for the new required `NfcAssignmentRepository` argument; 9 new `assignNfcTag` tests): an accepted-path test combining the full result-shape assertion with a `NfcAssignmentRepository.save`-called-once-with assertion (a disclosed, non-defective consolidation of what DT-023/DT-024 each split into two separate tests — full coverage is retained); the three `MembershipAuthorizationValidator` rejection paths (`membership_not_found`, `membership_lacks_administrator_role`, `cross_organization_access`), each with its own no-write assertion; an `NfcTag` Organization mismatch rejection; an `AssignmentTarget`/`Customer` Organization mismatch rejection; a missing-`Customer` rejection (proving Section 10.95's disclosed resolution is actually implemented); a consolidated spy assertion proving `NfcAssignmentRepository.save` is never called across all six rejection scenarios; and a deterministic-`NfcAssignmentId`-generation test using an injected fixed generator. `npm run test --workspace=@taptime/core` reports 40 test files and 219 tests passing (210 pre-existing + 9 new); `npm run typecheck --workspace=@taptime/core` is clean. A supplementary, out-of-band tests-inclusive `tsc --noEmit` check (Section 10.95) independently confirms zero type errors remain in this test file specifically. `MembershipAuthorizationValidator.test.ts`, `InMemoryCustomerRepository.test.ts`, `InMemoryNfcTagRepository.test.ts`, `InMemoryNfcAssignmentRepository.test.ts`, `AssignmentResolver.test.ts`, and `AssignmentValidator.test.ts` were all re-run and confirmed to pass with zero modification, verified additionally by an empty `git diff` against all six files and their corresponding source files. No composition-level or `apps/mobile` test exists for this capability — no UI, CLI, or mobile entry point calls `OrganizationAdministrationService`, per DT-025's own Out of Scope.

### 10.97 Common Implementation Pitfalls (Organization Administration: NFC Tag Assignment)

- **Positioning the new required `NfcAssignmentRepository` constructor parameter after a defaulted parameter** — the exact defect DT-024 produced one sprint earlier (Section 10.90); this sprint avoids it by having the plan specify the full constructor signature verbatim rather than in prose, and by independently re-verifying the result with the same tests-inclusive `tsc --noEmit` technique that originally caught the regression (Section 10.95).
- Calling `NfcTagRepository` inside `assignNfcTag(...)` to "double-check" the supplied `NfcTag` — `NfcTagRepository` has no lookup-by-identity method today, and the `NfcTag` argument is deliberately accepted as an already-resolved value; `assignNfcTag(...)` does not reference `this.nfcTagRepository` anywhere in its body, confirmed by direct inspection.
- Inventing a new rejection reason (e.g. importing `AssignmentValidationResult`'s `missing_assignment_target`) for the missing-target-Customer case instead of reusing `cross_organization_access` — Section 10.95 traces this directly to FB-002 Decision 5's own two-outcome evidence; the missing-target case is treated identically to an Organization mismatch, not given a distinct category.
- Checking `NfcAssignmentRepository.findActiveByTagId(...)` before saving, to "prevent" reassigning an already-assigned tag — Out of Scope, per DT-025's own Out of Scope ("re-assignment of an already-assigned tag... not resolved here"); `assignNfcTag(...)` performs no such check, confirmed by direct inspection.
- Implementing `assignNfcTag(...)` as a springboard to also implement DT-026's scan-pipeline verification "since all three methods now exist" — DT-026's own responsibility, explicitly a separate, later Development Task per `Development_Sprint_018_Plan.md` Section 4's reasoning.
- Modifying `MembershipAuthorizationValidator`, `AssignmentTarget`'s shape, or `NfcAssignmentRepository` beyond using its already-existing `save` method — DT-025's own Implementation Boundary states all three are consumed, not modified; confirmed unchanged by direct inspection.
- Wiring `OrganizationAdministrationService.assignNfcTag(...)` into a UI, CLI, or composition root "now that the service is complete" — Out of Scope, per DT-025's own Out of Scope; no caller was introduced for `assignNfcTag(...)` by this sprint.
- Modifying `MembershipAuthorizationValidator.test.ts`, `InMemoryCustomerRepository.test.ts`, `InMemoryNfcTagRepository.test.ts`, `InMemoryNfcAssignmentRepository.test.ts`, `AssignmentResolver.test.ts`, or `AssignmentValidator.test.ts` while adding nearby test coverage — confirmed to have zero diff against the pre-Sprint-018 commit; this was checked directly, not assumed.

### 10.99 Existing Scan Pipeline Integration Verification Implementation (DT-026)

Development Sprint 019 implements the eighth and final slice of FB-002/TS-002 (Organization Management Foundation), and the only one to add zero production code: a new composition-level test file, `packages/core/tests/application/OrganizationOwnedScanPipeline.test.ts` (245 lines, 2 tests) — commit `72e45f5`. Both tests construct every production class directly (`InMemoryOrganizationRepository`, `InMemoryMembershipRepository`, `InMemoryCustomerRepository`, `InMemoryNfcTagRepository`, `InMemoryNfcAssignmentRepository`, `InMemoryWorkEventRepository`, `InMemoryTimeEntryRepository`, `InMemoryOfflineQueue`, `OrganizationManagementService`, `MembershipService`, `MembershipAuthorizationValidator`, `OrganizationAdministrationService`, `AssignmentResolver`, `AssignmentValidator`, `WorkEventFactory`, `BusinessEngine`, `WorkEventCreationService`, `FakeNfcScanAdapter`, `NfcScanApplicationService`), with injected deterministic id/timestamp generators throughout — no mock exists anywhere in the file beyond a `vi.fn()` used for the `onEvent` callback parameter. The **accepted-path test** creates a real Organization A via `OrganizationManagementService.createOrganization('Org A')`; grants a real Administrator Membership and a real Employee Membership via `MembershipService.grantMembership(...)`; creates a real Customer, registers a real NFC Tag, and assigns it via `OrganizationAdministrationService.createCustomer(...)` → `registerNfcTag(...)` → `assignNfcTag(...)`; wires the *same* `customerRepository`/`nfcTagRepository`/`nfcAssignmentRepository` instances directly into `AssignmentResolver(nfcTagRepository, nfcAssignmentRepository)` and `AssignmentValidator(customerRepository)`; triggers a scan via `adapter.triggerScan('org-a-known-tag')` using the Employee Membership's own `userId`/`organizationId` to build `authenticatedCaller(...)`; calls `NfcScanApplicationService.submitScan(caller)`; and asserts `outcome.stage === 'validation'`, `outcome.result.status === 'accepted'`, exactly one saved `WorkEvent` whose `organizationId`/`assignmentId`/`nfcTagId`/`target` match the `NfcAssignment` `assignNfcTag(...)` produced, and a `TimeEntry` with `status: 'started'` for that target. The **cross-Organization rejection test** creates a second Organization B and its own Employee Membership, reuses Organization A's already-registered-and-assigned Tag payload unchanged, scans it with Organization B's caller, and asserts `outcome.result` equals `{ status: 'rejected', reason: 'employee_lacks_organization_access' }` with zero saved `WorkEvent`s.

### 10.100 Testing Strategy for Existing Scan Pipeline Integration Verification, and Why Zero Production Code Changed

`OrganizationOwnedScanPipeline.test.ts` (2 tests total, both new): the accepted-path test proves TS-002's own "Architecture Principles Preserved" claim directly, and the cross-Organization rejection test proves `AssignmentValidator`'s existing `employee_lacks_organization_access` rejection (unchanged since Development Sprint 001) continues to fire correctly against Administration-created data, with no new rejection reason invented. `npm run test --workspace=@taptime/core` reports 41 test files and 221 tests passing (219 pre-existing + 2 new); `npm run typecheck --workspace=@taptime/core` is clean. A supplementary, out-of-band tests-inclusive `tsc --noEmit` check independently confirms zero type errors are newly attributable to this file — the same 12 pre-existing, unrelated errors disclosed at Development Sprint 018's own closure (Section 10.95) remain, unchanged in count and location. `OrganizationAdministrationService.test.ts` (22 tests) and `NfcScanToTimeEntryPipeline.test.ts` (2 tests) were both re-run and confirmed to pass with zero modification, verified additionally by an empty `git diff` against both files. `runScan.ts`'s own existing CLI tests (`runScan.test.ts`, `runScan.callerOverride.test.ts`, `runScan.storageOverride.test.ts` — 14 tests total) were also re-run and confirmed passing with zero diff, satisfying DT-026's own Acceptance Criteria's explicit non-regression requirement for the demo CLI pipeline.
...
independent zero-finding correction review, exact-head ten-job CI and the complete fresh Galaxy
A33/NTAG213 Human gate. It did not add Membership authority, change C3C semantics or move any
Business Engine decision into a client.
...
- C3E1 identity-first Employee Membership setup has a zero-finding reviewed, Human-accepted
  invitation/redemption contract and repository implementation authority on `70d163f`; and
