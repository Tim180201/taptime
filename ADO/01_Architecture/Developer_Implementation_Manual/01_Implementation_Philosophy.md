...
### 10.16 Two Existing Patterns Recombine Without a Third Being Invented (Development Sprint 016)

Development Sprint 016 (DT-023) builds the first Organization Management Application Service to orchestrate both a Business-area validator and a repository write inside a single method — a materially different shape from `OrganizationManagementService`/`MembershipService` (Sections 10.12–10.13), which each construct-and-save with no authorization step at all. Rather than inventing a new orchestration idiom for this, `OrganizationAdministrationService.createCustomer(...)` is built entirely by recombining two precedents this repository already established independently: `NfcScanApplicationService`'s "resolve, branch on the result, write only on acceptance" shape (Section 5.4/2.3, "Application Orchestrates But Does Not Interpret"), and `OrganizationManagementService`'s/`MembershipService`'s constructor-with-injectable-deterministic-id-generation shape. Neither precedent alone was sufficient — `NfcScanApplicationService` has no id-generation concern, and `OrganizationManagementService`/`MembershipService` have no authorization branch — but composing them required no new pattern of their own, confirming this repository's existing Application Service vocabulary is expressive enough for a genuinely new combination of responsibilities without drifting into a bespoke shape.

### 10.17 An Idiom's Position, Not Just Its Presence, Is Part of Its Contract (Development Sprint 017)

Development Sprint 017 (DT-024) extends `OrganizationAdministrationService` with a second method, `registerNfcTag(...)`, built by recombining exactly the same two precedents Section 10.16 identifies for `createCustomer(...)` — no new orchestration idiom was needed, confirming Section 10.16's claim a second time rather than merely repeating it. What this sprint adds to the repository's own philosophy is a distinct, narrower finding, surfaced only because a real defect occurred: the injectable-deterministic-id-generation constructor idiom (`requiredDep1, requiredDep2, ..., newXId = default, newYId = default`) is not fully specified by naming its two parts — "required dependencies" and "defaulted id generators" — it also depends on required dependencies being contiguous and preceding all defaults, a structural property TypeScript's own compiler does not enforce for constructor bodies the way it enforces default-parameter-after-required-parameter rules for a single flat parameter list read left to right. Adding a third required dependency (`NfcTagRepository`) to a constructor that already had one defaulted parameter (`newCustomerId`) created exactly the case where "add the new dependency alongside the existing ones" is ambiguous between two positions, only one of which preserves callers' existing positional-argument contracts. This was not a violation of Section 5.4 (Extend Before Create) or Section 5.9 (Composability) in the abstract — both were followed, in that an existing idiom was extended rather than a new one invented — it is a reminder that following an idiom's *name* is not the same as preserving its *position*, and that this specific idiom's position-dependence is now an explicit, disclosed fact of this repository's philosophy rather than an implicit assumption, to be checked explicitly the next time this class gains a third required dependency (DT-025, `Development_Sprint_017_Closure.md` Section 15).

This sprint also reaffirms, in a new context, Section 10.9's "classifying an error is not the same as deciding one" family of findings, and Section 5.4/2.3's "Application Orchestrates But Does Not Interpret": the constructor-arity regression was a real, repository-verifiable defect (six `TS2554` errors under a tests-inclusive typecheck), not a matter of interpretation or judgment call, and it was corrected using the same discipline this repository already applies to every other kind of correction — evidence gathered first (reproducing the exact errors), a reasoned recommendation made and disclosed (reorder, not options-object, citing sibling-service precedent), and the fix independently re-verified using the same reproduction technique in reverse, rather than trusted on the strength of the correction having been applied.

### 10.18 A Lesson Applied Deliberately Is a Different Thing From a Lesson Merely Recorded (Development Sprint 018)

Development Sprint 018 (DT-025) completes `OrganizationAdministrationService`'s third method, `assignNfcTag(...)`, by recombining the same orchestration precedent Sections 10.16/10.17 already identify, plus one further recombination this sprint adds for the first time: rather than inventing a new strategy for verifying that an `AssignmentTarget` belongs to the requesting Membership's Organization, `assignNfcTag(...)` reuses `AssignmentValidator`'s own existing `CustomerRepository.findById(...)` resolution pattern verbatim — a Business-area component's internal resolution strategy, reused by an Application Service, with neither component modified. This is a second, independent instance of Section 10.16's "two existing patterns recombine without a third being invented" finding, now extended one step further: the reused precedent came from a component this service does not otherwise depend on architecturally (`AssignmentValidator` belongs to the FB-001 scan pipeline, not to Organization Management), confirming "Extend Before Create" (Section 5.4) applies across a repository's component boundaries, not only within a single service's own prior methods.

This sprint's more consequential finding, however, is what it demonstrates about Section 10.17's own lesson: a real regression was found and fixed there, but finding and fixing a defect once does not by itself prove the underlying lesson was internalized — only a second, independent application under the same conditions does. Development Sprint 018 added a fourth required constructor dependency to the exact same class, under the exact same structural risk (a class with both required dependencies and defaulted parameters, gaining one more required dependency), and this time the constructor's exact signature was specified verbatim in the plan rather than described in prose. The result, confirmed by this sprint's own out-of-band tests-inclusive typecheck, is zero arity-related errors. This is recorded here as a distinct, positive finding, not merely a restatement of Section 10.17: a disclosed lesson only becomes a repository property, rather than a one-off correction, once it changes how the next, structurally similar task is planned — and this sprint is the direct evidence that it did.

This sprint also produces a second, independent confirmation of Section 10.14's "structurally identical means shape and contract, not a copied constructor" lesson, this time applied to a result type rather than a class constructor: `CreateCustomerResult` reuses `RejectedMembershipAuthorizationResult` verbatim for every rejection case rather than inventing a new `AdministrativeActionRejected` category FB-002's own product-level Decision 3 prose might have suggested in isolation. TS-002 Sequence Diagram 3 is the more precise, implementation-facing evidence that settles this: it names the exact three rejection reasons `MembershipAuthorizationResult` already defines, confirming that reuse — not invention — is the evidence-supported choice, with only the minimum new accepted-branch shape added for what the caller actually needs (the created `Customer` and its event).

### 10.19 Proof Over Assertion: Closing an Architectural Claim With a Passing Test, Not Design-Document Language Alone (Development Sprint 019)
...
- treat the C3E1 invitation/redemption design as a review proposal only, never as implementation
  authority; and
- keep C3E2 reassignment, broader Membership management and production authority behind separate
  product/policy decisions.

All non-C3 administration-independent gates listed in Section 11 remain. This addendum changes no
accepted implementation philosophy and remains pending independent ADO review.
