...
**Status: E1 prerequisite implemented and Technical-Lead/GitHub-CI approved; independent review pending (2026-07-14).** E1 replaces Block D's volatile ambiguous-command slot with one strict device-only, User/Organization-bound lifecycle outbox record and exact C2/B6 replay. Implementation commit `e0f2898` passed all eight jobs in run `29340810743`. It advances DT-060–DT-062 but does not complete them. Full offline scan-context caching, multiple events, scheduling/backoff, background sync and DT-063–DT-068 remain open and separately gated.
