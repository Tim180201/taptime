...
Date: 2026-07-10
Status: Active execution baseline — Block A, B1–B6 and C1/C2 completed after Technical Lead, GitHub CI and independent security approval as of 2026-07-14; Block D software and device-local NFC validation approved/passed, synthetic server-connected harness Technical-Lead approved after six corrections with latest correction awaiting CI and physical product-path validation in progress; C3 and Block E unauthorized
...
**Status: Software Approved and Device-local NFC Validation Passed — Synthetic Server-connected Harness Technical-Lead Approved after Six Corrections, Latest Correction Awaiting GitHub CI; Physical Product-path Validation In Progress (2026-07-14).** V1 uses only the versioned canonical Android UID payload `nfc:uid:v1:<UPPERCASE_HEX>`. The real `NfcScanPort.scan()` drives the private C2 server path with single-flight timeout/cancellation cleanup, physical-discovery timestamp binding, session-generation isolation, protected same-evidence ambiguous retry and truthful UI presentation. The Galaxy A33 5G / Android 15 / two-NTAG213 device-local checklist passed. The five-test local harness proves synthetic Auth, C2, PostgreSQL and genuine Core Start/Stop without LAN/tunnel/cloud. Review added native-project preservation, scoped USB reverse cleanup, dedicated PostgreSQL-17 CI enforcement, its clean-Linux dependency build sequence, strict host-loopback database execution and exact restoration of Expo's tracked prebuild rewrite. A 66-MB release APK has now built for the ADB-authorized SM-A336B; installation and physical unassigned/Start/Stop observations remain open. Core/Mobile pass 288/253.
