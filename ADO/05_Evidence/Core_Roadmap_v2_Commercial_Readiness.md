...
Date: 2026-07-10
Status: Active execution baseline — Block A and B1–B6 completed; Block C1 Technical Lead approved and awaiting CI plus independent security review as of 2026-07-13; C2/C3 unauthorized
...
**Status: Architecture approved; B1–B6 completed after Technical Lead, GitHub CI and independent security approval (2026-07-13); C1 real runtime/auth is Technical Lead approved and awaiting CI plus independent security review.** ADR-0008 was approved after independent architecture/security review and corrective Technical Lead verification. B1's managed-Node transaction/security spike passed 39 direct-PostgreSQL tests locally and in GitHub Actions after two security correction rounds. B2 completed the Promise migration across all twelve effectful ports and passed GitHub Actions run `29221790966` after one Technical Lead correction. B3 provides three versioned PostgreSQL 17 migrations, twelve forced-RLS tables, reciprocal TimeEntry/Decision traceability, truthful five-way Core Decision/SyncReceipt mappings, exact Start/Stop WorkEvent `occurred_at` binding and its 125-test current negative matrix. B4 adds migration `004` plus the isolated Node 24 identity workspace: the corrected migration normalizes an existing resolver role and removes parent-role contamination; JWT verification binds JWKS to the exact issuer path and requires HTTPS outside numeric loopback tests; and current active server Membership data authoritatively supplies User, Organization and role. Its local suite passes 54 B4 tests; implementation commit `570fc0b` passed GitHub Actions run `29261459523`; the independent Claude review returned `APPROVED` with no P0/P1 findings. B5 adds the isolated Node 24 `@taptime/backend-read-model` workspace: one cohesive `READ COMMITTED READ ONLY` transaction reuses the concrete B4 verifier, resolves current Membership, compares requested Organization, selects a fixed Employee/Administrator role and provides exactly five tenant-qualified read-only Core-port projections under B3 RLS. Reader capabilities expire when the callback settles. The Technical Lead closed one pre-publication pool-capability lifetime defect. Forty-one direct PostgreSQL/JWT tests pass locally and implementation commit `68b7f44` passed all five jobs in GitHub Actions run `29264083804`; the independent Claude review returned `APPROVED`. B6 adds the non-HTTP `@taptime/backend-lifecycle` boundary and migration `005`: locked current authority/configuration, fixed lifecycle role, User serialization, active-TimeEntry lock, unchanged genuine Core evaluation and atomic exact five-way evidence. The final 67-case matrix includes all three authoritative event-time validity starts and historical-deferred retry semantics. Implementation commit `9531672` passed all six jobs in GitHub Actions run `29269282536`; the independent Claude review returned `APPROVED`. C1 implements DT-045/046 and DT-049–052 as one vertical slice: default product Mobile composition, explicit lazy demo isolation, Supabase email/password, refresh-token-only native restoration, B4-backed session/Membership resolution through one private route and injected UI session capability. Technical Lead review added bounded dependency deadlines; its local matrix passes 43 C1 and 95 Mobile tests. DT-047/048, C2/C3, B5/B6 HTTP transports, Supavisor validation and production personal data remain gated.
...
**Status: C1 Technical Lead approved (2026-07-13) for DT-045/046 and DT-049–052; awaiting CI plus independent security review; DT-047/048 and C2/C3 remain separately gated.** The default Mobile path now uses the real Supabase email/password adapter, refresh-token-only SecureStore restoration and server-authoritative Membership session resolution; the fake/demo scan graph is explicit, lazy and development-only. The product stops truthfully at authenticated-but-scan-not-yet-connected. C2 requires separate authorization for B5/B6 transport composition; C3 requires separate authorization for Organization/Admin bootstrap.
