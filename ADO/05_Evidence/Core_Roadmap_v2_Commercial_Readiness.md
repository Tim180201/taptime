...
Date: 2026-07-10
Status: Active execution baseline — Block A, B1–B6 and C1 completed after Technical Lead, GitHub CI and independent security approval as of 2026-07-14; C2/C3 unauthorized
...
**Status: Architecture approved; B1–B6 and C1 completed after Technical Lead, GitHub CI and independent security approval (2026-07-14); C2/C3 remain unauthorized.** ADR-0008 was approved after independent architecture/security review and corrective Technical Lead verification. B1's managed-Node transaction/security spike passed 39 direct-PostgreSQL tests locally and in GitHub Actions after two security correction rounds. B2 completed the Promise migration across all twelve effectful ports and passed GitHub Actions run `29221790966` after one Technical Lead correction. B3 provides three versioned PostgreSQL 17 migrations, twelve forced-RLS tables, reciprocal TimeEntry/Decision traceability, truthful five-way Core Decision/SyncReceipt mappings, exact Start/Stop WorkEvent `occurred_at` binding and its 125-test current negative matrix. B4 adds migration `004` plus the isolated Node 24 identity workspace: the corrected migration normalizes an existing resolver role and removes parent-role contamination; JWT verification binds JWKS to the exact issuer path and requires HTTPS outside numeric loopback tests; and current active server Membership data authoritatively supplies User, Organization and role. Its implementation commit `570fc0b` passed GitHub Actions run `29261459523`; the independent Claude review returned `APPROVED`. B5 adds the isolated Node 24 `@taptime/backend-read-model` workspace: one cohesive `READ COMMITTED READ ONLY` transaction reuses the concrete B4 asymmetric verifier, resolves current Membership, compares requested Organization, selects a fixed Employee/Administrator role and provides five tenant-qualified read-only Core-port projections under B3 RLS. Its implementation commit `68b7f44` passed GitHub Actions run `29264083804`; the independent Claude review returned `APPROVED`. B6 adds the non-HTTP `@taptime/backend-lifecycle` boundary and migration `005`: locked current authority/configuration, fixed lifecycle role, User serialization, active-TimeEntry locking, unchanged genuine Core evaluation and atomic exact five-way evidence. Implementation commit `9531672` passed GitHub Actions run `29269282536`; the independent Claude review returned `APPROVED`. C1 implements DT-045/046 and DT-049–052: default product Mobile composition, explicit lazy demo isolation, Supabase email/password, refresh-token-only native restoration, B4-backed session/Membership resolution through one private route and injected UI session capability. Technical Lead review added bounded dependency deadlines; implementation commit `0a2a51b` passed all seven jobs in GitHub Actions run `29279475547`. The independent verdict was `APPROVED WITH NON-BLOCKING FINDINGS`; its shared-JWKS-infrastructure observation is dispositioned and protected by new B4/B5/B6 regressions, bringing their current totals to 55/42/68. C1 retains 43 backend and 95 Mobile cases. DT-047/048, C2/C3, B5/B6 HTTP transports, Supavisor validation and production personal data remain gated.
...
**Status: C1 completed (2026-07-14) for DT-045/046 and DT-049–052 after Technical Lead, GitHub CI and independent security approval; DT-047/048 and C2/C3 remain separately gated.** The default Mobile path now uses the real Supabase email/password adapter, refresh-token-only SecureStore restoration and server-authoritative Membership session resolution; the fake/demo scan graph is explicit, lazy and development-only. The product stops truthfully at authenticated-but-scan-not-yet-connected. C2 requires separate authorization for B5/B6 transport composition and must preserve infrastructure-versus-authorization error truth; C3 requires separate authorization for Organization/Admin bootstrap.
